...
#
# Adds memory_search_semantic tool that finds memories by meaning, not just
# substring. Uses Cloudflare Workers AI (bge-m3 multilingual embedding) +
# Vectorize (vector DB). Requires CF_ACCOUNT_ID + CF_API_TOKEN env vars; if
# absent, all CF helpers no-op gracefully and the existing memory_search tool
# remains the only search path.
#
# Multi-project: one shared Vectorize index, namespaced via metadata.project.
# Each query filters to PROJECT_ROOT.name so cross-project bleed is impossible.
#
# Embedding model bge-m3: 1024-dim multilingual (Thai/EN/code mix friendly).
# Index spec: cosine metric, 1024 dim. Create once via:
#   wrangler vectorize create goat-memory-search --dimensions=1024 --metric=cosine

CF_ACCOUNT_ID = os.environ.get("CF_ACCOUNT_ID", "").strip()
CF_API_TOKEN  = os.environ.get("CF_API_TOKEN", "").strip()
CF_VECTORIZE_INDEX = os.environ.get("CF_VECTORIZE_INDEX", "goat-memory-search").strip()
CF_EMBED_MODEL = "@cf/baai/bge-m3"
CF_EMBED_DIM = 1024
PROJECT_TAG = PROJECT_ROOT.name  # e.g. 'GOAT_StockMGT', 'Mem_MCP_v1'
...
def _embed_text(text: str) -> Optional[List[float]]:
    """Embed text via Workers AI bge-m3. Returns 1024-dim vector or None on failure.

    Truncates to 8000 chars (model context ~8K tokens). Network/auth errors are
    swallowed and logged via stderr — callers must handle None.
    """
    if not _cf_enabled() or not text:
        return None
    try:
        url = f"{_cf_base()}/ai/run/{CF_EMBED_MODEL}"
        payload = {"text": [text[:8000]]}
        r = httpx.post(url, headers=_cf_headers(), json=payload, timeout=15.0)
        r.raise_for_status()
        data = r.json()
        # bge-m3 returns {result: {data: [[...vector...]], shape: [1, 1024]}, success: true}
        result = data.get("result", {})
        vec = (result.get("data") or [None])[0]
        if not vec or len(vec) != CF_EMBED_DIM:
            return None
        return vec
    except Exception as e:
        import sys
        print(f"[mem-mcp][embed] failed: {e}", file=sys.stderr)
        return None

def _vec_text_for_memory(m: dict) -> str:
    """Build the text we embed per memory — title carries strong signal, content
    is body, tags are repeated for keyword-like boost."""
    title = m.get("title", "") or ""
    content = m.get("content", "") or ""
    tags = " ".join(m.get("tags") or [])
    mtype = m.get("memory_type", "") or ""
    # Title twice: matches well against query intent.
    return f"{title}\n\n{title}\n\n[{mtype}] tags: {tags}\n\n{content}"

def _vec_metadata_for_memory(m: dict) -> dict:
    """Vectorize metadata stays small — enough to filter + display, full content
    hydrated from JSON store on hit. 10KB total budget per Vectorize doc."""
    return {
        "project": PROJECT_TAG,
        "agent_name": (m.get("agent_name") or "")[:100],
        "memory_type": m.get("memory_type") or "",
        "title": (m.get("title") or "")[:200],
        "created_at": m.get("created_at") or "",
    }

def _vectorize_upsert(vectors: List[dict]) -> tuple[int, Optional[str]]:
    """Upsert a batch of vectors. Each vector dict: {id, values, metadata}.
    Returns (count_succeeded, error_or_None). Vectorize accepts up to 1000
    vectors per upsert call.
    """
    if not _cf_enabled() or not vectors:
        return (0, "CF not configured" if not _cf_enabled() else None)
    try:
        url = f"{_cf_base()}/vectorize/v2/indexes/{CF_VECTORIZE_INDEX}/upsert"
        # Vectorize v2 upsert wants NDJSON body — one vector per line.
        body = "\n".join(json.dumps(v, ensure_ascii=False) for v in vectors)
        headers = {**_cf_headers(), "Content-Type": "application/x-ndjson"}
        r = httpx.post(url, headers=headers, content=body.encode("utf-8"), timeout=30.0)
        r.raise_for_status()
        data = r.json()
        count = (data.get("result") or {}).get("mutationId")
        # mutationId means accepted; counting is approximate.
        return (len(vectors), None) if count else (0, "no mutationId in response")
    except Exception as e:
        return (0, str(e))

def _vectorize_query(vector: List[float], top_k: int = 10, project: Optional[str] = None) -> List[dict]:
    """Query top-k similar vectors. Filter by metadata.project for project isolation.

    Returns list of {id, score, metadata} ranked by score desc. Empty list on
    failure or no matches.
    """
    if not _cf_enabled() or not vector:
        return []
    try:
        url = f"{_cf_base()}/vectorize/v2/indexes/{CF_VECTORIZE_INDEX}/query"
        payload = {
            "vector": vector,
            "topK": min(max(top_k, 1), 100),
            "returnMetadata": "all",
        }
        if project:
            payload["filter"] = {"project": {"$eq": project}}
        r = httpx.post(url, headers=_cf_headers(), json=payload, timeout=15.0)
        r.raise_for_status()
        data = r.json()
        ...
        import sys
        print(f"[mem-mcp][vec-query] failed: {e}", file=sys.stderr)
        return []

def _vectorize_delete(ids: List[str]) -> tuple[int, Optional[str]]:
    """Delete vectors by id list. Vectorize accepts up to 1000 ids per call."""
    if not _cf_enabled() or not ids:
        return (0, None)
    try:
        url = f"{_cf_base()}/vectorize/v2/indexes/{CF_VECTORIZE_INDEX}/delete-by-ids"
        r = httpx.post(url, headers=_cf_headers(), json={"ids": ids}, timeout=30.0)
        r.raise_for_status()
        return (len(ids), None)
    except Exception as e:
        return (0, str(e))
...
@mcp.tool(name="memory_search_semantic", annotations={"title":"Semantic Search (Vectorize)","readOnlyHint":True,"destructiveHint":False,"idempotentHint":True,"openWorldHint":True})
async def memory_search_semantic(params: SearchInput) -> str:
    """Find memories by MEANING via Cloudflare Vectorize + bge-m3 embeddings.

    Use when keyword search misses paraphrases (e.g. 'fix the silent drift' →
    surfaces 'Carbon+ reconciliation' even though those words don't overlap).
    Returns matches scored by cosine similarity, filtered to the current
    project. Falls back to a clear error if CF env vars are not configured
    — callers should use memory_search instead.
    """
    if not _cf_enabled():
        return "⚠️ Semantic search disabled: set CF_ACCOUNT_ID + CF_API_TOKEN in MCP env.\nUse `memory_search` for substring search instead."
    qvec = _embed_text(params.query)
    if not qvec:
        return f"⚠️ Embedding failed for '{params.query}'. Check stderr logs and CF API token permissions (needs Workers AI:Run + Vectorize:Edit)."
    matches = _vectorize_query(qvec, top_k=params.limit, project=PROJECT_TAG)
    if not matches:
        return f"No semantic matches for '{params.query}' in project '{PROJECT_TAG}'.\nIf the index has never been populated, run `memory_reindex` first."
    # Hydrate full memory rows from local JSON by id.
    by_id = {m["id"]: m for m in _load_mem()}
    lines = [f"# 🧠 Semantic '{params.query}' — {len(matches)} results (project: {PROJECT_TAG})\n"]
    for match in matches:
        ...
        full = by_id.get(mid)
        if full:
            lines.append(f"### [{full['memory_type'].upper()}] {full['title']}  *(score: {score:.3f})*")
            lines.append(f"*✍️ {full['agent_name']}*\n{full['content'][:300]}{'...' if len(full.get('content',''))>300 else ''}\n---\n")
        else:
            # Memory was indexed but no longer in local JSON — stale Vectorize entry.
            lines.append(f"### [STALE] {meta.get('title','(unknown)')}  *(score: {score:.3f})*")
            lines.append(f"*Vector id `{mid}` not found in local store — reindex recommended.*\n---\n")
    return "\n".join(lines)

@mcp.tool(name="memory_reindex", annotations={"title":"Reindex Memories to Vectorize","readOnlyHint":False,"destructiveHint":False,"idempotentHint":True,"openWorldHint":True})
async def memory_reindex() -> str:
    """Embed every memory in this project and upsert to Vectorize. Idempotent.

    Run once after first configuring CF_ACCOUNT_ID + CF_API_TOKEN, then again
    after batches of writes (the v1 design does NOT auto-embed on memory_write
    — keeps the write path fast and offline-resilient).

    Vectorize cost: ~1-2 neurons per memory embedded (free tier 10K/day).
    """
    if not _cf_enabled():
        return "⚠️ Cannot reindex: set CF_ACCOUNT_ID + CF_API_TOKEN in MCP env first."
    mem = _load_mem()
    if not mem:
        return f"No memories to index in project '{PROJECT_TAG}'."
    embedded = 0
    failed = 0
    batch: List[dict] = []
    for m in mem:
        text = _vec_text_for_memory(m)
        ...
        batch.append({
            "id": m["id"],
            "values": vec,
            "metadata": _vec_metadata_for_memory(m),
        })
        embedded += 1
        # Flush every 100 — well under the 1000 per-upsert ceiling, keeps
        # individual NDJSON requests small enough to fit comfortably under
        # API timeouts.
        if len(batch) >= 100:
            count, err = _vectorize_upsert(batch)
            if err:
                return f"❌ Upsert failed mid-batch ({embedded} embedded, {failed} failed): {err}"
            batch = []
    if batch:
        count, err = _vectorize_upsert(batch)
        if err:
            return f"❌ Final upsert failed ({embedded} embedded, {failed} failed): {err}"
    return (
        f"✅ Reindexed project '{PROJECT_TAG}' to Vectorize index '{CF_VECTORIZE_INDEX}'.\n"
        f"• Embedded: {embedded}\n"
        f"• Failed:    {failed}\n"
        f"• Total memories: {len(mem)}\n\n"
        f"Test with `memory_search_semantic` using a paraphrased query."
    )
